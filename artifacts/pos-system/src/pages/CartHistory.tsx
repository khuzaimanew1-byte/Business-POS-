...
import {
  ArrowLeft, ChevronDown, ChevronRight, Clock, ShoppingCart, TrendingUp,
} from "lucide-react";
...
// ── Atom: empty state ──────────────────────────────────────────────────────
// Large cart icon with a small clock badge in the bottom-right corner. The
// optional third line is shown on desktop only (matches the design spec).
function EmptyState({ withResetHint }: { withResetHint?: boolean }) {
  return (
...
export default function CartHistory() {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      {/* ── HEADER ──────────────────────────────────────────────────── */}
      <header className="h-14 sm:h-16 flex items-center gap-3 px-3 sm:px-6 border-b border-border bg-background/90 backdrop-blur-sm shrink-0 sticky top-0 z-10">
        <button
          onClick={() => setLocation("/")}
        ...
        {/* Mobile-only count badge — desktop subsumes the count into the
            header subtitle to keep the right edge clean. */}
        {orders.length > 0 && (
          <div
      ...
      {/* ── BODY ────────────────────────────────────────────────────── */}
      {orders.length === 0 ? (
        // Empty state — single layout, the third hint line shows on every
        // viewport since both empty cards in the spec are essentially
        // identical except for the desktop's extra "resets at 7 AM" line.
        // We render it everywhere; on mobile the bottom strip still shows
        // a 0-profit row so the page never feels truncated.
        <main className="flex-1 flex flex-col">
          <div className="flex-1 flex items-center justify-center">
            <EmptyState withResetHint />
          </div>
          <div className="sm:hidden border-t border-border px-4 py-3 flex items-center justify-between bg-background shrink-0">
        ...
        <main className="flex-1 flex flex-col sm:flex-row min-h-0">
          {/* ── MOBILE: collapsible list + bottom profit strip ─────── */}
            ...
            <div className="border-t border-border px-4 py-3 flex items-center justify-between bg-background shrink-0">
              <div className="flex flex-col">
                ...
                <span
                  ...
                  data-testid="text-todays-profit-mobile"
                >
                  {fmtCur(totalProfit)}
                </span>
              </div>
          ...
          {/* ── DESKTOP: left list (with profit strip) + right detail ── */}
          <aside className="hidden sm:flex flex-col w-[320px] border-r border-border min-h-0 shrink-0">
            <div className="px-4 pt-4 pb-2 text-[10px] font-semibold uppercase tracking-[0.14em] text-muted-foreground">
              Orders
            </div>
            <ScrollArea className="flex-1">
            ...
            <div className="border-t border-border px-4 py-3 flex items-center justify-between shrink-0">
              <div className="flex flex-col">
                ...
                <span
                  className="text-lg font-bold text-primary tabular-nums"
                  data-testid="text-todays-profit-desktop"
                >
                  {fmtCur(totalProfit)}
                </span>
              </div>
            ...
            {selectedOrder && (
              <>
                ...
                <div className="border-t border-border px-6 py-4 flex items-center justify-between shrink-0">
                  ...
                  >
                    {fmtCur(selectedOrder.totalSales)}
                  </span>
                </div>
              </>
            )}
